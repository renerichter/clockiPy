...
class TestSummaryTables(unittest.TestCase):
    ...
    def test_subproject_table_has_plan_columns(self, mock_stdout):
        """Test that the SubProject table includes Over and Under columns."""
    ...
    def test_project_table_has_plan_columns(self, mock_stdout):
        """Test that the Project table includes Over and Under columns."""
        # Create report generator
        report_generator = ReportGenerator(self.time_entries, "2023-01-01", "normal")
    ...
    def test_tag_table_has_plan_columns(self, mock_stdout):
        """Test that the Tag table includes Over and Under columns."""
        # Create report generator
        report_generator = ReportGenerator(self.time_entries, "2023-01-01", "normal")
    ...
    @patch('sys.stdout', new_callable=StringIO)
    def test_spontaneity_table_has_plan_columns(self, mock_stdout):
        """Test that the Spontaneity table includes Over and Under columns."""
        # Create report generator
        report_generator = ReportGenerator(self.time_entries, "2023-01-01", "normal")
        ...
        # Check if the output contains the new columns in the Spontaneity table
        output = mock_stdout.getvalue()
        self.assertIn("### Spontaneity", output)
        self.assertIn("Under %", output)
        self.assertIn("Over %", output)

class TestTimeDifferenceCalculations(unittest.TestCase):
    """Test the time difference calculation logic (Over Plan, Under Plan, Abs)."""
    ...
    def test_measured_greater_than_planned(self):
        """When measured > planned, should contribute to Over Plan."""
    ...
    def test_measured_less_than_planned(self):
        """When measured < planned, should contribute to Under Plan."""
