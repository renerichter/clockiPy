...
class ReportGenerator:
    ...
    def _generate_entries_table(self, output: StringIO, csv_prefix: Optional[str] = None, day_table: bool = False):
        ...
        # Process entries for Dur-Plan column
        task_groups = defaultdict(list)
        for idx, entry in enumerate(self.entries):
            task_groups[entry.description].append(idx)
        ...
        for task_name, indices in task_groups.items():
            planned_sec = self.task_planned_measured[task_name]["planned"]
            total_measured = self.task_planned_measured[task_name]["measured"]
    ...
    def _generate_subproject_table(self, output: StringIO, csv_prefix: Optional[str] = None):
        ...
        for entry in self.entries:
            ...
            # Add plan differences for this task to the subproject
            planned = self.task_planned_measured[entry.description]["planned"]
            measured = self.task_planned_measured[entry.description]["measured"]
            
            if planned > 0:  # Only consider tasks with planned time
                diff = measured - planned
                if diff > 0:  # More measured than planned
                    subproject_plan_diffs[key]["more_plan"] += diff
                elif diff < 0:  # Less measured than planned
    ...
    def _generate_project_table(self, output: StringIO, csv_prefix: Optional[str] = None):
        ...
        for task_name, data in self.task_planned_measured.items():
            for entry in self.entries:
                if entry.description == task_name:
                    ...
                    break  # Only process each task once
    ...
    def _generate_tag_table(self, output: StringIO, csv_prefix: Optional[str] = None):
        ...
        for entry in self.entries:
            ...
            if planned > 0:  # Only consider tasks with planned time
                diff = measured - planned
                # Distribute the difference proportionally to the duration for each tag
                for tag in entry.tag_names:
                    if diff > 0:  # More measured than planned
                        tag_plan_diffs[tag]["more_plan"] += diff / len(entry.tag_names)
                    elif diff < 0:  # Less measured than planned
                        tag_plan_diffs[tag]["less_plan"] += abs(diff) / len(entry.tag_names)
    ...
    def _generate_spontaneousity_table(self, output: StringIO, csv_prefix: Optional[str] = None):
        ...
        for entry in self.entries:
            if entry.is_spontaneous or entry.is_scheduled:
                symbol = "🎲" if entry.is_spontaneous else "🗓️"
                planned = self.task_planned_measured[entry.description]["planned"]
                measured = self.task_planned_measured[entry.description]["measured"]
    ...
    def _generate_totals_table(self, output: StringIO, csv_prefix: Optional[str] = None):
        ...
        for task_name, data in self.task_planned_measured.items():
            ...
            if diff > 0:
                over_plan_total += diff
            elif diff < 0:
                under_plan_total += abs(diff)
